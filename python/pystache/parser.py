...
class _PartialNode(object):
    ...
    def render(self, engine, context):
        cname = type(context.top()).__name__
        template = engine.resolve_partial(cname)
...
class _SectionNode(object):
    ...
    def __init__(self, key, parsed, delimiters, template, index_begin, index_end, separator):
        self.delimiters = delimiters
        self.key = key
        self.parsed = parsed
    ...
    def render(self, engine, context):
        ...
        for val in values:
            ...
            if self.separator is not None:
                separator = engine.fetch_string(context, self.separator)
...
class _Parser(object):
    ...
    _delimiters = None
    _template_re = None
    _separator = defaults.SEPARATOR

    def __init__(self, delimiters=None,separator=None):
    ...
    def _make_section_node(self, template, tag_type, tag_key, parsed_section,
        ...
        if tag_type == '#':
            return _SectionNode(tag_key, parsed_section, self._delimiters,
                               template, section_start_index, section_end_index, None)

        if tag_type == '%':
            return _SectionNode(tag_key, parsed_section, self._delimiters,
                               template, section_start_index, section_end_index, self._separator)
        if tag_type == '^':
